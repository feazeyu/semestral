...
{
    [Serializable]
    public class InventoryList : MonoBehaviour, IScrollHandler, IUIPositionalItemContainer
    {
        ...
        public void OnScroll(PointerEventData eventData)
        {
            ...
            float overflowSize = -rectTransform.sizeDelta.y + contents.Count*(slotPrefabRect.sizeDelta.y+margin.y);
            if (overflowSize > 0)
            {
                firstElementPosition += new Vector2(0, eventData.scrollDelta.y * scrollSensitivity); // Adjust the scroll sensitivity as needed
        ...
        public void GenerateUI()
        {
            ...
            foreach (StackableInventorySlot slot in contents)
            {
                DrawSlotUI(slot, i);
                i++;
            }
        ...
        public bool PutItem(Vector2Int position, GameObject item)
        {
            ...
            if (EnableSlotCapacity)
            {
                ...
                {
                    existingSlot = contents[position.y];
                }
